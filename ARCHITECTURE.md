...
- **Role-gating no front é parcialmente enforceado.** `ProtectedRoute` valida papel contra a rota (CLIENT→`/portal/cliente/*`, ORGANIZER→`/portal/organizador/*`, PORTARIA→`/portal/portaria/*`), redirecionando para `/portal` se o papel não bater. A rota `/portal/perfil` é compartilhada entre todos os papéis. Mas a autorização real é feita pelos guards no backend.
